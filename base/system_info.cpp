...
uint64_t SystemInfo::total_physical_memory_size()
{
...
uint64_t SystemInfo::avialable_physical_memory_size()
{
	MEMORYSTATUSEX memory_status;
	memory_status.dwLength = sizeof(memory_status);
	if (GlobalMemoryStatusEx(&memory_status))
...
bool SystemInfo::is_x64_system()
{
	...
	return (sys_info.wProcessorArchitecture == PROCESSOR_ARCHITECTURE_AMD64
		|| sys_info.wProcessorArchitecture == PROCESSOR_ARCHITECTURE_ALPHA64
		|| sys_info.wProcessorArchitecture == PROCESSOR_ARCHITECTURE_IA64
		|| sys_info.wProcessorArchitecture == PROCESSOR_ARCHITECTURE_IA32_ON_WIN64);
}

std::wstring SystemInfo::ie_version()
{
	CRegKey value_key;
	std::wstring key = L"SOFTWARE\\Microsoft\\Internet Explorer";
	wchar_t buffer[1024] = { 0 };
	DWORD len = 0;
	if (ERROR_SUCCESS == value_key.Open(HKEY_LOCAL_MACHINE, key.c_str(), is_x64_system()?(KEY_READ | KEY_WOW64_64KEY): KEY_READ))
	{
		LSTATUS ret = 0;
		len = _countof(buffer);
		ret = value_key.QueryStringValue(L"svcVersion", buffer, &len);
		if (ret == 0)
		{
			return buffer;
		}
		...
		len = _countof(buffer);
		ret = value_key.QueryStringValue(L"Version", buffer, &len);
		if (ret == 0)
		{
			return buffer;
		}
