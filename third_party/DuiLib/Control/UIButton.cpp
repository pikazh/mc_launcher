...
Label_ForeImage:
	...
	void CButtonUI::PaintBkColor(HDC hDC)
	{
		if ((m_uButtonState & UISTATE_PUSHED) != 0) {
			if (m_dwPushedBkColor != 0) {
				if (m_dwPushedBkColor >= 0xFF000000) CRenderEngine::DrawColor(hDC, m_rcPaint, GetAdjustColor(m_dwPushedBkColor));
				else CRenderEngine::DrawColor(hDC, m_rcItem, GetAdjustColor(m_dwPushedBkColor));
			}
		}
		else if ((m_uButtonState & UISTATE_HOT) != 0) {
			if (m_dwHotBkColor != 0) {
				if (m_dwHotBkColor >= 0xFF000000) CRenderEngine::DrawColor(hDC, m_rcPaint, GetAdjustColor(m_dwHotBkColor));
	...
	void CButtonUI::SetHotBorderColor(DWORD color)
	{
		m_dwHotBorderColor = color;
		m_dwNormalBorderColor = m_dwBorderColor;
	}
